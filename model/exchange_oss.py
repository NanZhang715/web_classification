...
base_dir = '/home/nzhang/exhange_model/serving/1556261101'
mark_record = './params/mark_record_exchange.json'
#positove folder
#local_folder ='/datanfs/rzx/toinner/p2p_snapshot/' 
local_folder ='datanfs/rzx/exchange_snapshot/snapshot'
...
#    raw_data.columns
    for raw_data in generator_df:
        ...
        snapshot_files = list(map(lambda s: re.sub('/share_disk/', '', s, count=1), snapshot_files))
...
#        web_df = read_snapshot(snapshot_files)
        ...
        # drop columns
        data.drop(['offset','corpus', 'keywords', 'description','meta'], axis=1, inplace =True)
        ...
        if output_df.empty:
            print('No positive spoted')
            logging.info('_'*50)
            continue

        # stratey rules to extract exchange platform
        output_df = output_df[output_df['title'].str.contains('交易', regex=True)]       
