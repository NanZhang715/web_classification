...
if __name__ == '__main__':
    ...
    inputs_sql = '''select
               ...
               from 
                     producer_snapshot
               where  id > {} and snapshot is not NULL limit 2048  
             ;'''.format(mark_id)
...
#        output_df['top_domain'] = output_df['domain'].map(lambda s: getDomainShortNameByEnd(s))
        ...
        if output_df.empty:
            logging.info('No postive data from main classifier, the task is terminated')
            continue
        ...
        prediction_fn = predictor.from_saved_model(export_dir=sub_dir)
        logging.info('sub_model has loaded, print result in on-going...')
        result_sub =print_result(list(map(prediction_fn,input_sub)))
        logging.info('print result is done, the model result columns are {}'.format(result.columns))
